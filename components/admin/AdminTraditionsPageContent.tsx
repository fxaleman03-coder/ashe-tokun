...
import type { TraditionReadResult } from "@/lib/data/traditions";

type AdminTraditionsPageContentProps = {
  traditions: TraditionReadResult["traditions"];
  source: TraditionReadResult["source"];
};

export default function AdminTraditionsPageContent({
  traditions,
  source,
}: AdminTraditionsPageContentProps) {
  const { t } = useLanguage();
  const labels = t.admin.traditions;
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {traditions.map((tradition) => (
          <ManagementCard
            ...
            description={tradition.description ?? common.pending}
            meta={tradition.slug}
            status={tradition.active ? common.active : common.inactive}
            action={common.view}
          />
        ))}
      </section>
