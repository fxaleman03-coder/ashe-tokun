...
import type { CollectionReadResult } from "@/lib/data/collections";

type AdminCollectionsPageContentProps = {
  collections: CollectionReadResult["collections"];
  source: CollectionReadResult["source"];
};

export default function AdminCollectionsPageContent({
  collections,
  source,
}: AdminCollectionsPageContentProps) {
  const { t } = useLanguage();
  const labels = t.admin.collections;
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {collections.map((collection) => (
          <ManagementCard
            ...
            description={collection.description ?? common.pending}
            meta={
              collection.featured ? common.featuredCollection : collection.slug
            }
            status={collection.active ? common.active : common.inactive}
            action={common.view}
          />
        ))}
      </section>
