...
type FeaturedProductsPreviewClientProps = {
  products: Product[];
};

export default function FeaturedProductsPreviewClient({
  products,
}: FeaturedProductsPreviewClientProps) {
  const { language, t } = useLanguage();
  const overrides = useProductOverrides();
  const featuredProducts = useMemo(
    () =>
      mergeProductCatalog(products, overrides).filter(
